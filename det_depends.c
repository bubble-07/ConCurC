...
file_depends_result_dynarray load_deps(file_depends_result in) {
    ...
    for (i=0; i < filerefs.size; i++) {
        tmp = det_file_deps(primorder(parse(lex(filerefs.begin[i]))));
        result = file_depends_result_dynarray_add(result, tmp);
    }
    return result;
}

file_depends_result_graph load_file_and_depends(path in) {
    file_depends_result firstfile = det_file_deps(primorder(parse(lex(in))));

...
static type##_tree type##_tree_##state_t##_dfmap(type##_tree in, \
                                        type (*transform)(type, type##_tree_dynarray, state_t), \
                                        state_t initial)  {\
    size_t i; \
    for (i=0; i < in.children.size; i++) { \
 in.children.begin[i] = type##_tree_##state_t##_dfmap(in.children.begin[i], transform, initial); \
    } \
    in.data = transform(in.data, in.children, initial); \
    return in; \
} \
\
static type##_tree type##_tree_##state_t##_hfmap(type##_tree in, \
                    type##_tree_dynarray (*transform) (type##_tree_dynarray, type, state_t), \
                    state_t initial) { \
    in.children = transform(in.children, in.data, initial); \
    size_t i; \
    for (i=0; i < in.children.size; i++) { \
  in.children.begin[i] = type##_tree_##state_t##_hfmap(in.children.begin[i], transform, initial);\
    } \
    return in; \
}

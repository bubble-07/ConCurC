...
string get_name_from_def(lexid_tree_dynarray def_expr, string_dynarray backsymtable) {
    ...
    if (!lexid_eq(typing_expr.begin[0].data, EXPR_LEXID)) {
        ...
        return backsymtable.begin[typing_expr.begin[0].data.tokenval];
    }
    //Otherwise, it must be "function returntype"
    lexid_tree_dynarray fun_ret_pair = typing_expr.begin[0].children;
    return backsymtable.begin[fun_ret_pair.begin[1].data.tokenval];
}

//Collect every function that's defined with "DEF"
void populate_global_table(lexid_tree in, string_dynarray backsymtable) {
    ...
    for (i=0; i < defs.size; i++) {
        ...
        if (lexid_eq(def_expr.begin[0].data, DEF_LEXID)) {
            ...
            if (lexid_eq(def_expr.begin[1].data, EXPR_LEXID)) {
                ...
                string function_name = get_name_from_def(def_expr, backsymtable);

                printf("%s", to_cstring(function_name));
...
parse_result collectnames(parse_result in) {
    ...
    populate_global_table(in.AST, in.backsymtable);

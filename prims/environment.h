...
env add_param_to_env(env in, parameter_ptr p) {
    lexid name = p->name;
    in.binds = lexid_parameter_ptr_dict_quickadd(name, p);
    return in;
}
...
env add_params_to_env(env in, parameter_dynarray toadd) {
    ...
    for (i=0; i < toadd.size; i++) {
        ...
        in = add_param_to_env(parameter_dynarray_getptr(toadd, i));
    }
    return in;
}
...
parameter_ptr env_lookup(env in, lexid name) {
    ...
    parameter_ptr result = lexid_parameter_ptr_get(in.binds, name);
